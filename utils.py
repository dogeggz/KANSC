...
from models.mutual_info import mutual_information, sample_batch
...
class BleuScore:
    """
    Compute BLEU score for evaluating the quality of text generation
    """
...
class LabelSmoothing(nn.Module):
    "Implement label smoothing. to prevent the model from becoming over-confident"
...
class NoamOpt:
    """
    A custom learning rate scheduler that implements a warmup and decay strategy
    """
...
class Channels:
    """
    Simulates different communication channels
    """
...
def initNetParams(model):
    """Init net parameters."""
    """Initalizes the network using Xavier uniform initialization"""
...
def loss_function(x, trg, padding_idx, criterion):
    """
    Calculates the loss while ignoring the padding
    """
...
def PowerNormalize(x):
    """
    Normalizes the power of the signal
    """
...
def SNR_to_noise(snr):
    """
    Converts SNR to noise variance
    """
    snr = 10 ** (snr / 10)
...
def train_step(model, src, trg, n_var, pad, opt, criterion, channel, mi_net=None):
    ...
    b, t, _ = dec_output.shape
    pred = model.kan(dec_output.reshape(-1, dec_output.shape[-1])).reshape(
        b, t, model.trg_vocab_size
    ) if hasattr(model, 'kan') else model.dense(dec_output)
...
def train_mi(model, mi_net, src, n_var, padding_idx, opt, channel):
    """
    Train mutual information model
    """
    mi_net.train()
    opt.zero_grad()
    channels = Channels()
...
# validation step that returns the loss for current model
def val_step(model, src, trg, n_var, pad, criterion, channel):
    """
    Performs a validation step to evaluate the model
    """
    channels = Channels()
    trg_inp = trg[:, :-1]
    trg_real = trg[:, 1:]
